...
function buildCarCard(car) {
  ...
  const delBtn = currentUser ? `
    <button class="admin-only delete-car-btn" onclick="deleteCar('${car.id}')">
      ${S.deleteBtn}
    </button>` : '';
  ...
  card.innerHTML = `
    ...
    <div class="card-body">
      <h3>${esc(carName)}</h3>
      <div class="price-tag">€${carPrice} <span>${S.perDay}</span></div>
      ${delBtn}
      <button class="reserve-btn" onclick="openModal('${car.id}','${esc(carName)}',${carPrice})">
        ${S.reserveBtn}
      </button>
    </div>`;
...
function filterCars(term) {
  ...
  gallery.querySelectorAll('.carCard').forEach(card => {
    const show = !t || (card.dataset.name || '').includes(t);
  ...
  updateCount(visible);
  if (noResults) {
    noResults.style.display = (t && visible === 0) ? 'block' : 'none';
    if (termSpan) termSpan.textContent = term || '';
  }
}

// Wire search inputs after DOM ready
document.addEventListener('DOMContentLoaded', () => {
  const si = document.getElementById('searchInput');
  if (!si) return;
  si.addEventListener('input', () => filterCars(si.value));
  si.addEventListener('keydown', e => {
    if (e.key === 'Enter' && !document.getElementById('carGallery')) {
      const val = si.value.trim();
      if (val) window.location.href = `cars.html?search=${encodeURIComponent(val)}`;
    }
  });
});
